...
pub const Font = struct {
    ...
    const Glyph = struct {
        ...
        fn scaleOutput(glyph: *Glyph, scale: f32) void {
            glyph.xy = glyph.xy.scale(scale);
    ...
    fn queryFnNk(font_ptr: c.nk_handle, font_height: f32, glyph: ?*c.nk_user_font_glyph, codepoint: c.nk_rune, next_codepoint: c.nk_rune) callconv(.C) void {
        ...
        glyph.?.* = .{
            .width = gg.size.get(.x),
            .height = gg.size.get(.y),
            .offset = .{ .x = gg.offset.get(.x), .y = gg.offset.get(.y) },
            .xadvance = gg.xadvance,
            .uv = .{
                .{ .x = gg.uv[0].get(.x), .y = gg.uv[0].get(.y) },
                .{ .x = gg.uv[1].get(.x), .y = gg.uv[1].get(.y) },
            },
        };
    }
    ...
    fn widthFn(font_ptr: ?*anyopaque, height: f32, txt: []const u8) f32 {
        ...
        while (it.nextCodepointSlice()) |slice| {
            ...
            var glyph = font.findGlyph(codepoint).*;
            glyph.scaleOutput(height / font.baked_font.height);
    ...
    fn queryFn(font_ptr: ?*anyopaque, height: f32, codepoint: u21, next_codepoint: u21) UserFont.Glyph {
        ...
        return .{
            .offset = glyph.xy.swizzle("xy"),
            .size = .init(
                glyph.xy.get(.z) - glyph.xy.get(.x),
                glyph.xy.get(.w) - glyph.xy.get(.y),
            ),
...
pub const FontAtlas = struct {
    ...
    pub const TTFAddInfo = struct {
        ...
        spacing: g.Vec2 = .zero,
        ranges: []const CodepointRange = &.{.{ .start = 0x20, .end = 0x100 }},
        fallback_codepoint: u21 = '?',
    };
            ...
            for (baker_font.ranges) |range| {
                for (0..@intCast(range.num_chars)) |char_idx| {
                    ...
                    baker.atlas.glyphs[
                        ...
                        .xy = .init(
                            q.x0,
                            q.y0 + config.baked_font.ascent + 0.5,
                            q.x1,
                            q.y1 + config.baked_font.ascent + 0.5,
                        ),
                        .uv = .init(
                            q.s0 * scale_x,
                            q.t0 * scale_y,
                        ...
                        .xadvance = if (config.pixel_snap)
                            @round(pc.xadvance + config.spacing.get(.x))
                        else
                            pc.xadvance + config.spacing.get(.x),
                    };
                }
            }
        }
